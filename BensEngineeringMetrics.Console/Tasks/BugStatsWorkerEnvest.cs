...
public class BugStatsWorkerEnvest(IJiraQueryRunner runner, ICsvExporter exporter, IWorkSheetUpdater sheetUpdater, IOutputter outputter)
{
    private const string CustomerFilter = """AND ("Customer/s (Multi Select)[Select List (multiple choices)]" = Envest OR "Exalate[Short text]" ~ Envest) """;
    ...
    private async Task ExportBugStatsCategories(List<JiraIssue> jiras)
    {
        ...
        exporter.SetFileNameMode(FileNameMode.ExactName, $"{this.keyString}-Envest-Categories");
    ...
    private async Task ExportBugStatsCodeAreas(List<JiraIssue> jiras)
    {
        ...
        exporter.SetFileNameMode(FileNameMode.ExactName, $"{this.keyString}-Envest-Areas");
    ...
    private async Task ExportBugStatsRecentDevelopment(List<JiraIssue> jiras)
    {
        ...
        exporter.SetFileNameMode(FileNameMode.ExactName, $"{this.keyString}-Envest-RecentDev");
    ...
    private async Task ExportBugStatsReportedVsBacklog(List<BarChartData> severityTotals)
    {
        ...
        exporter.SetFileNameMode(FileNameMode.ExactName, $"{this.keyString}-Envest-ReportVsBacklog");
    ...
    private async Task ExportBugStatsReportedVsResolved(List<BarChartData> severityTotals)
    {
        ...
        exporter.SetFileNameMode(FileNameMode.ExactName, $"{this.keyString}-Envest-ReportVsResolved");
    ...
    private async Task<List<BarChartData>> ExportBugStatsSeverities(List<JiraIssue> jiras, string? customerFilter = null)
    {
        ...
        if (customerFilter is null)
        {
            ...
            exporter.SetFileNameMode(FileNameMode.ExactName, $"{this.keyString}-Envest-Severities");
            var fileName = exporter.Export(bugCounts, overrideSerialiseRecord: SerialiseToCsv);
